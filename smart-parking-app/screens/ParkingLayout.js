...
const ParkingLot = () => {
    ...
    useEffect(() => {
        const fetchParkingStatus = () => {
            fetch('http://192.168.8.48:5000/api/parking-status')
    ...
    // Split spaces into left and right columns
                ...
                <View style={styles.column}>
                    {leftColumn.map((space, index) => (
                        <View
                            ...
                            style={[
                                styles.space,
                                index < 3 ? styles.handicap : // First 3 spaces are handicap
                                space.status === 'occupied' ? styles.occupied :
                                space.status === 'available' ? styles.available :
                                space.status === 'reserved' && styles.reserved, // Grey for reserved
                            ]}
                        >
...
const styles = StyleSheet.create({
    ...
    handicap: {
        ...
        borderColor: '#FFA500', // Optional: Orange border for handicap spaces
    },
    reserved: {
        backgroundColor: 'gray', // Grey for reserved
    },
    text: {
        color: 'white',
        fontWeight: 'bold',
